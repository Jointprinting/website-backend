...
test('Client stage enum matches the agreed vocabulary (order matters)', () => {
  ...
  assert.deepEqual(enumVals, ['lead', 'contacted', 'quoting', 'won', 'customer', 'lost', 'dormant']);
});
...
test('board columns + probabilities match the frontend mirror (order matters)', () => {
  assert.deepEqual(BOARD_COLUMNS, ['lead', 'contacted', 'quoting', 'approval', 'production', 'shipped', 'delivered']);
